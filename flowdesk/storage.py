...
class Store:
    ...
    def initialize(self):
        ...
        try:
            ...
            if 1 not in versions:
                # The initial migration is additive. Future destructive migrations
                # must call backup() before opening their write transaction.
                for statement in SCHEMA_001.split(";"):
                    if statement.strip():
    ...
    def create_project(self, name="Untitled project", content=None, evidence=None, views=None):
        ...
        try:
            ...
            if evidence:
                ...
                connection.execute("CREATE TABLE IF NOT EXISTS detected_symbols (id TEXT PRIMARY KEY, project_id TEXT NOT NULL REFERENCES projects(id) ON DELETE CASCADE, data TEXT NOT NULL)")
    ...
    def backup(self):
        ...
        destination = directory / f"flowdesk-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')}.sqlite3"
        source, target = self.connect(), sqlite3.connect(destination)
        try:
