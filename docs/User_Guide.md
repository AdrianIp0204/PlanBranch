...
Conversation, comments, proposals, question sets, submitted answers, and approval records live outside diagram undo history. They survive restarts and are included in SQLite backups, but are intentionally omitted from portable JSON and Markdown exports. Accepted edits become ordinary content and are exported normally. Unsent chat, node comments and partially answered question forms save separately to the database. They recover after browser or server restart and never send automatically. Acknowledged submission retires only that writing version; newer typing remains. Conflicting tabs retain both versions in **Unsent writing**, with explicit use, copy and discard recovery. Writing whose node or question was removed remains available to copy or discard. Failed saves remain visible and block project navigation until recovered. Undo does not reopen replaced question sets.
...
Each run uses a separate Git worktree created from the repository's committed revision. Your checkout and its uncommitted changes are excluded. One task runs at a time; closing the dialog does not cancel it. **Cancel run** stops the owned process tree and retains its files. Restarting PlanBranch marks unfinished runs Interrupted. Open **Execution history** and **Refresh results** to inspect preserved work; a restart never repeats the agent request automatically.

Review the changed files and observed command results. **Agent report** is the model's account; only **Observed commands** records captured exit codes and output. A successful agent response does not imply that tests passed. Failed checks remain visible and require your judgment.

- **Accept changes** records acceptance of the exact reviewed result.
- **Complete task** separately adds a completion action to manual history. Undo restores the task status. Completion makes plan approval outdated; approve the updated plan before the next run.
- **Apply to checkout** separately copies accepted changes to the selected repository after confirmation. Conflicting staged or working files, a changed source commit, or changed task requirements block application. An interrupted Apply retains a journal for an explicit retry. It never stages, commits, merges or pushes.
